...
#Function solve CNFs using pysat
def solve_minesweeper_CNF(CNF, rows, cols,matrix):
    #Create a solver and put all clause from the CNF in solver
    solver = Glucose3()
    for clause in CNF:
        solver.add_clause(clause)

    #Let the pysat sovle the CNF
    solution = solver.solve()

    #If it can solved, make the output matrix base on the solution
    if solution:
...
CNF = generate_minesweeper_CNF(input_matrix)

#Solve the CNF
start_time = time.time()
solved_matrix = solve_minesweeper_CNF(CNF,rows,cols,input_matrix)
end_time = time.time()
